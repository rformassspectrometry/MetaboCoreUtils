test_that("Migration time conversion works", {
  ...
  µ_s1 <- convertMT(rtime, marker, method = "single", U = 30, L = 90, 
                    markerID = "marker1")
  ...
  expect_error(convertMT(y = marker, method = "single", 
                         U = 30, L = 90, markerID = "marker1"), 
               "Missing vector 'x' with migration times")
  expect_error(convertMT(rtime, method = "single", 
                         U = 30, L = 90, markerID = "marker1"), 
               "Missing data.frame 'y' with marker information")
  expect_error(convertMT(rtime, marker[,-1], method = "single", 
                         U = 30, L = 90, markerID = "marker1"), 
               "'y' requires exact three columns")
  expect_error(convertMT(rtime, marker_E, method = "single", 
                         U = 30, L = 90, markerID = "marker1"), 
               "Missing column 'markerID', 'rtime', 'mobility' or all")
  expect_error(convertMT(rtime, marker, U = 30, L = 90, markerID = "marker1"), 
               "'method' is missing")
  expect_error(convertMT(rtime, marker, method = c("single","multiple"), 
                         U = 30, L = 90, markerID = "marker1"), 
               "Conversion method is either 'single' or 'multiple'")
  expect_error(convertMT(rtime, marker, method = "X", 
                         U = 30, L = 90, markerID = "marker1"), 
               "Conversion method is either 'single' or 'multiple'")
  expect_error(convertMT("X", marker, method = "single", 
                         U = 30, L = 90, markerID = "marker1"), 
               "'x' needs to be numeric")
  expect_error(convertMT(rtime, marker_E2, method = "single", 
                         U = 30, L = 90, markerID = "marker1"), 
               "'mobility' entries in 'y' needs to be numeric")
  expect_error(convertMT(rtime, marker_E3, method = "single", 
                         U = 30, L = 90, markerID = "marker1"), 
               "'rtime' entries in 'y' needs to be numeric")
  expect_error(convertMT(rtime, marker, method = "single", 
                         L = 90, markerID = "marker1"), 
               "'U' is missing")
  expect_error(convertMT(rtime, marker, method = "single", 
                         U = 30, markerID = "marker1"), 
               "'L' is missing")
  expect_error(convertMT(rtime, marker, method = "single", 
                         U = 30, L = 90), 
               "'markerID' is missing")
  expect_error(convertMT(rtime, marker[-c(1:2),], method = "single", 
                         U = 30, L = 90, markerID = "marker1"), 
