test_that("beta_values works", {
    res <- beta_values(1:10, zero.rm = FALSE)
    expect_true(is.numeric(res))
    expect_equal(names(res), c("best_cor", "beta_snr"))
    expect_lt(res[1L], 0.0001)
    expect_lt(res[2L], 2)
    res <- beta_values(1:10, zero.rm = TRUE)
    expect_true(is.numeric(res))
    expect_equal(names(res), c("best_cor", "beta_snr"))
    expect_lt(res[1L], 0.0001)
    expect_lt(res[2L], 2)
...
test_that("beta_values with real peak data", {
    ...
    res_2 <- beta_values(skinny_peak[20:40], skinny_peak_rt[20:40])
